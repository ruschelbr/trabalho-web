...
async function findById(request, response) {
  try {
    const result = await model.findByPk(request.params.id, {
      include: { model: album, attributes: ["name", "cover"] },
    })
    if (result == null) {
...
async function findSongsOfAlbum(request, response) {
  //método que deve ser chamado para a página de discografia, coloca numa lista os 3 albums e chama isso pra cada um na hora
  //de mostrar
